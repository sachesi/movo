...
/// languages it serves.
pub(super) const SEASON_LABELS: [&str; 2] = ["season", "сезон"];
pub(super) const EPISODE_LABELS: [&str; 5] = ["episode", "серия", "серія", "эпизод", "епізод"];
...
pub fn parse_details_html(html: &str, url: &str) -> Result<MediaDetails, String> {
    ...
    Ok(MediaDetails {
        ...
        schedules: parse_schedules(&document),
...
#[cfg(test)]
mod tests {
    use super::{labelled_position, mark_scheduled, parse_details_html, parse_episodes_html};
