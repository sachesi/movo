...
impl StreamExtractor {
    pub async fn fetch_movie_stream(
        ...
        let json_resp = session
            ...
            .await?;
        let mut bundle = Self::parse_stream_json(&json_resp, post_id, tr_id, None, None)?;
        Self::absolutize_storyboard(session, &mut bundle);
        Self::load_storyboard(session, &mut bundle).await;
        Ok(bundle)
    }
    ...
    pub async fn fetch_episode_stream(
        ...
        let json_resp = session
            ...
            .await?;
        let mut bundle =
            Self::parse_stream_json(&json_resp, post_id, tr_id, Some(season), Some(episode))?;
        Self::absolutize_storyboard(session, &mut bundle);
        Self::load_storyboard(session, &mut bundle).await;
        Ok(bundle)
    }
    ...
    pub fn parse_stream_json(
        ...
        let decoded_str = match parsed.get("streams") {
            Some(Value::Object(streams_map)) => {
                ...
                if parts.is_empty() {
                    Self::decoded_url(&parsed)?
                } else {
                    parts.join(",")
                }
            }
            _ => Self::decoded_url(&parsed)?,
        };
        let streams = Self::parse_stream_entries(&decoded_str);
        ...
        if let Some(Value::String(sub_str)) = parsed.get("subtitle") {
            ...
            let lns = parsed
                ...
                .unwrap_or_default();
            subtitles = Self::parse_subtitles_with_meta(sub_str, &default_code, &lns);
        }
    ...
    async fn load_storyboard(session: &RezkaSession, bundle: &mut StreamBundle) {
        ...
        if let Ok(vtt) = session.get_html(url).await {
            bundle.storyboard = Self::parse_storyboard(&vtt, url);
        }
    }
    ...
    fn parse_storyboard(vtt: &str, source_url: &str) -> Vec<StoryboardCue> {
        ...
        for window in lines.windows(2) {
            ...
            let Some(start_ms) = Self::parse_timestamp(start) else {
                continue;
            };
            let Some(end_ms) = Self::parse_timestamp(end) else {
                continue;
            };
    ...
    pub fn parse_stream_entries(decoded: &str) -> Vec<StreamEntry> {
        ...
        for chunk in chunks {
            ...
            let quality = Self::clean_quality_label(raw_q);
    ...
    pub fn parse_subtitles(raw_sub: &str) -> Vec<SubtitleTrack> {
        Self::parse_subtitles_with_meta(raw_sub, "", &HashMap::new())
    }
    ...
    pub fn parse_subtitles_with_meta(
        ...
        for (name, url) in raw_tracks {
            ...
            tracks.push(SubtitleTrack {
                code: name.clone(),
                title: Self::map_subtitle_code_to_label(&name),
                url,
                language_code,
                is_default,
            });
...
#[cfg(test)]
mod tests {
    use super::{StreamExtractor, BASE64_STANDARD, NOISE_MARKER};
    use base64::Engine;
    ...
    fn decodes_a_payload_split_by_noise_runs() {
        ...
        for pieces in 1..=8 {
            assert_eq!(
                StreamExtractor::decrypt_streams(&obfuscate(plain, pieces)),
                plain,
    ...
    fn decodes_a_payload_whose_padding_was_dropped() {
        ...
        assert_eq!(StreamExtractor::decrypt_streams("#hYWJjZA"), "abcd");
    }

    #[test]
    fn returns_a_plain_url_untouched() {
        let url = "https://cdn.example.org/video/1080.mp4";
        assert_eq!(StreamExtractor::decrypt_streams(url), url);
    }

    #[test]
    fn parses_storyboard_cues_and_resolves_relative_images() {
        let cues = StreamExtractor::parse_storyboard(
