...
pub async fn fetch_catalog(
    ...
    let items = parse_catalog_html(&html_content);
    Ok(items)
}
...
pub fn parse_catalog_html(html: &str) -> Vec<MediaItem> {
    ...
    for element in document.select(&item_selector) {
        let (title, url, id) = if let Some(link) = element.select(&link_selector).next() {
            ...
            let id = extract_id_from_url(&href);
            (title, href, id)
        } else {
            continue;
        };
        ...
        let year = info.as_ref().and_then(|info_text| extract_year(info_text));
...
#[cfg(test)]
mod tests {
    use super::parse_catalog_html;

    #[test]
    fn catalog_uses_secure_lazy_poster_url() {
        let items = parse_catalog_html(
