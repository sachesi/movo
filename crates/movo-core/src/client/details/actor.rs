...
pub async fn fetch_actor(session: &RezkaSession, url: &str) -> Result<ActorDetails, String> {
    ...
    let mut actor = parse_actor_html(&html);
...
fn parse_actor_html(html: &str) -> ActorDetails {
    ...
    let mut actor = ActorDetails {
        ...
        birth_place: None,
        height: None,
        films: crate::client::catalog::parse_catalog_html(html),
        roles: document
            ...
            .map(|role| ActorRole {
                ...
                info: role
                    ...
                    .map(|node| node.text().collect::<String>().trim().to_string())
                    .unwrap_or_default(),
                films: crate::client::catalog::parse_catalog_html(&role.html()),
            })
            .collect(),
    };
...
#[cfg(test)]
mod tests {
    use super::parse_actor_html;

    #[test]
    fn parses_actor_pages() {
        let actor = parse_actor_html(
