...
impl AnubisSolver {
    ...
    pub async fn extract_and_solve(
        ...
        let challenge_json = Self::extract_script_tag(html, "anubis_challenge")
            .ok_or_else(|| "Missing anubis_challenge tag in HTML".to_string())?;

        let prefix_json = Self::extract_script_tag(html, "anubis_base_prefix")
        ...
        let (nonce, hash) = match algorithm {
            AnubisAlgorithm::Fast | AnubisAlgorithm::Slow => {
                Self::solve_pow(random_data, difficulty)
            }
