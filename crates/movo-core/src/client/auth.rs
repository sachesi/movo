...
pub async fn login(
    ...
    if !success {
        ...
        return Err(sanitize_message(msg));
    }
    ...
    let mut profile = match fetch_verified_profile(session, &user_id).await {
        Ok(profile) => profile,
        Err(error) => {
            let _ = session.clear_session(None);
            return Err(error);
...
pub async fn check_profile(session: &RezkaSession) -> Result<Option<UserProfile>, String> {
    ...
    fetch_verified_profile(session, &user_id).await.map(Some)
}
...
async fn fetch_verified_profile(
    ...
    let mut profile = parse_verified_profile(&profile_html, user_id)?;
...
fn parse_verified_profile(html: &str, user_id: &str) -> Result<UserProfile, String> {
    ...
    Ok(UserProfile {
        ...
        username,
        is_logged_in: true,
        is_vip: extract_vip_status(html),
        email,
        avatar_url,
        premium_days: None,
...
pub async fn fetch_notifications(
    ...
    require_authenticated_html(session, &html)?;
    Ok(parse_notifications(&html))
}
...
pub async fn fetch_favorites_categories(
    ...
    require_authenticated_html(session, &html_content)?;
    Ok(parse_favorites_categories(&html_content))
}

pub fn parse_favorites_categories(html_content: &str) -> Vec<FavoritesCollection> {
    let doc = Html::parse_document(html_content);
    let item_sel = Selector::parse(".b-favorites_content__cats_list_item[data-cat_id]").unwrap();
...
pub async fn add_to_favorites(
    ...
    let json_resp = session
        ...
        .await?;
    require_json_success(&json_resp, "Failed to update favorites")
}
        ...
        .filter_map(|item| {
            ...
            Some(ServerHistoryEntry {
                ...
                is_watched: item
                    .value()
                    .attr("class")
                    .is_some_and(|classes| classes.split_whitespace().any(|c| c == "watched-row")),
            })
        })
        .collect()
}
...
pub async fn remove_history(session: &RezkaSession, id: &str) -> Result<(), String> {
    let response = session
        ...
        .await?;
    require_json_success(&response, "Failed to remove history item")
}
...
pub async fn toggle_history_watched(session: &RezkaSession, id: &str) -> Result<(), String> {
    let response = session
        ...
        .await?;
    require_json_success(&response, "Failed to update watched state")
}
...
pub async fn toggle_schedule_watched(session: &RezkaSession, id: &str) -> Result<(), String> {
    let response = session
        ...
        .await?;
    require_json_success(&response, "Failed to update episode watched state")
}
...
mod tests {
    ...
    fn parses_notifications_and_premium_days() {
        let (groups, days) = parse_notifications(
            r#"
    ...
    async fn login_requires_cookies_and_verified_profile() {
        ...
        let profile = login(&session, "tester", "password").await.unwrap();
    ...
    async fn save_watch_accepts_provider_false_response_for_later_verification() {
        ...
        let session = RezkaSession::new_for_test(&base_url);
        save_watch(&session, 7, 8, Some(1), Some(2)).await.unwrap();
        server.join().unwrap();
    }
    ...
    fn account_parsers_reject_landing_page_and_parse_server_state() {
        assert!(
            parse_verified_profile("<html><head><title>HDRezka</title></head></html>", "42")
                .is_err()
        );

        let categories = parse_favorites_categories(
